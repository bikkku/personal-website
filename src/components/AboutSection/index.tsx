    ...
    <div className="w-full h-full flex flex-col">
      ...
      <div className="border border-2 border-teal-dark bg-mint-light rounded-4xl p-8 mt-4 flex flex-col lg:flex-row justify-between gap-12">
        <div className="flex flex-col">
          ...
          <div className="border border-2 border-teal-dark rounded-lg w-full grid  grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-x-2 bg-mint mt-4">
            {skills.map((skill) => {
              return (
                <div
                  key={skill.id}
                  className="flex group w-full max-w-36 aspect-square bg-mint-light rounded-xl border-teal-dark border-3 m-4 text-center items-center mx-auto"
                >
                  <div className="flex flex-col m-auto">
                    <img
                      src={`/svg/skills/${skill.src}.svg`}
                      className="w-14 h-14 stroke-teal mx-auto"
                    />
                    <Text className="text-md text-teal-dark font-bold font-mono">
                      {skill.label}
                    </Text>
                  </div>
                </div>
